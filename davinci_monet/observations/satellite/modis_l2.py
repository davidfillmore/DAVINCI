"""MODIS L2 observation reader with swath-to-grid binning.
...
def build_modis_variable_dict(
    ...
    for var_name, cfg in variables.items():
        ...
        if scale != 1.0:
            entry["scale"] = scale
        obs_min = cfg.get("obs_min")
        if obs_min is not None:
            entry["minimum"] = obs_min
        obs_max = cfg.get("obs_max")
        if obs_max is not None:
            entry["maximum"] = obs_max
...
class MODISL2Reader:
    ...
    ...     files, variable_dict,
    ...     lat_centers=model_lat, lon_centers=model_lon,
    ...
    def read_granules(
        ...
        *,
        ...
        return modis_mod.read_mfdataset(file_list, variable_dict, debug=debug)
    ...
    def _grid_granules(
        ...
        *,
        ...
        for i, (granule_key, granule) in enumerate(granules.items()):
            ...
            # Shift lon from -180..180 to 0..360 if model grid is 0..360
        ...
        if min_obs_count > 1:
            for var_name in var_names:
                data_grids[var_name][count_grids[var_name] < min_obs_count] = np.nan
        ...
        # Use the maximum count across variables for obs_count
        ...
        data_vars["obs_count"] = (["time", "lon", "lat"], max_count)
        ...
        ds.attrs["source_obs_type"] = "sat_swath_clm"
