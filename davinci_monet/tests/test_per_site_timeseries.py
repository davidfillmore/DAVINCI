...
def synthetic_paired_data() -> xr.Dataset:
    ...
    ds = xr.Dataset(
        {
            "x_o3": (["site", "time"], x_data),
            "y_o3": (["site", "time"], y_data),
        },
        coords={
            "site": sites,
            "time": times,
    ...
    ds["x_o3"].attrs["units"] = "ppb"
    ds["y_o3"].attrs["units"] = "ppb"
    return ds
...
def sparse_paired_data() -> xr.Dataset:
    ...
    ds = xr.Dataset(
        {
            "x_pm25": (["site", "time"], x_data),
            "y_pm25": (["site", "time"], y_data),
        },
        coords={
            "site": sites,
            "time": times,
...
class TestPerSiteTimeSeriesPlotter:
    ...
    def test_plot_returns_figure(self, synthetic_paired_data: xr.Dataset) -> None:
        ...
        fig = plotter.plot(synthetic_paired_data, "x_o3", "y_o3", min_points=5)
    ...
    def test_line_geometry_style(self, synthetic_paired_data: xr.Dataset) -> None:
        plotter = PerSiteTimeSeriesPlotter()
        fig = plotter.plot(
            synthetic_paired_data,
            "x_o3",
    ...
    def test_scatter_dataset_style(self, synthetic_paired_data: xr.Dataset) -> None:
        plotter = PerSiteTimeSeriesPlotter()
        fig = plotter.plot(
            synthetic_paired_data,
            "x_o3",
