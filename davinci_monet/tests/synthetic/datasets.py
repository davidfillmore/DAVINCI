"""Synthetic gridded dataset generators.

This module generates synthetic atmospheric gridded data (CMAQ-like,
WRF-Chem-like) for testing pairing and analysis components.
"""
...
from davinci_monet.tests.synthetic.geometries import (
...
@dataclass
class DatasetConfig:
    """Configuration for synthetic gridded data.
...
def create_dataset_dataset(
    ...
    """Create a synthetic gridded dataset.

    The dataset mimics atmospheric chemistry gridded data
    ...
    -------
    xr.Dataset
        Synthetic gridded dataset.

    Examples
    --------
    >>> ds = create_dataset_dataset(variables=["O3"], n_levels=0)
    >>> "O3" in ds
    True
    >>> "time" in ds.dims
    True
    ...
    ds.attrs = {
        "title": "Synthetic Dataset Output",
...
def create_dataset_dataset_from_config(config: DatasetConfig) -> xr.Dataset:
    """Create a synthetic gridded dataset from a configuration object.
...
def create_surface_dataset(
    variables: Sequence[str] | None = None,
    domain: Domain | None = None,
    time_config: TimeConfig | None = None,
    seed: int | None = 42,
) -> xr.Dataset:
    """Create surface-only gridded data with no vertical levels.
    ...
    -------
    xr.Dataset
        Surface gridded dataset with dims (time, lat, lon).
    """
    return create_dataset_dataset(
        variables=variables,
        domain=domain,
...
def create_3d_dataset(
    ...
    """Create 3D gridded data with vertical levels.

    Convenience function for 3D dataset data.
    ...
    -------
    xr.Dataset
        3D dataset dataset with dims (time, level, lat, lon).
    """
    return create_dataset_dataset(
        variables=variables,
        domain=domain,
