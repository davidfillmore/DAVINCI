...
This module generates synthetic datasets for different geometries:
...
def create_point_geometries(
    ...
    """Create synthetic point datasets (surface stations).
    ...
    -------
    xr.Dataset
        Point dataset dataset with geometry attribute.

    Examples
    --------
    >>> ds = create_point_geometries(n_sites=5, variables=["O3"])
    >>> ds.dims
    Frozen({'time': ..., 'site': 5})
    >>> ds.attrs["geometry"]
    'point'
    ...
    ds.attrs = {
        "title": "Synthetic Point Datasets",
        "geometry": "point",
...
def create_track_geometries(
    ...
    """Create synthetic track datasets (aircraft).
    ...
    -------
    xr.Dataset
        Track dataset dataset with geometry attribute.
    """
    if variables is None:
        variables = ["O3", "CO"]
    if domain is None:
    ...
    ds.attrs = {
        "title": "Synthetic Track Datasets",
        "geometry": "track",
        "source": "davinci_monet.tests.synthetic",
        "platform": "aircraft",
    }
...
def create_profile_geometries(
    ...
    """Create synthetic profile datasets (sondes).
    ...
    -------
    xr.Dataset
        Profile dataset dataset with geometry attribute.
    """
    if variables is None:
    ...
    ds.attrs = {
        "title": "Synthetic Profile Datasets",
        "geometry": "profile",
        "source": "davinci_monet.tests.synthetic",
        "platform": "ozonesonde",
    }
...
def create_swath_geometries(
    ...
    """Create synthetic swath datasets (satellite L2).
    ...
    -------
    xr.Dataset
        Swath dataset dataset with geometry attribute.
    """
    if variables is None:
        variables = ["NO2"]
    if domain is None:
    ...
    ds.attrs = {
        "title": "Synthetic Swath Datasets",
        "geometry": "swath",
        "source": "davinci_monet.tests.synthetic",
        "platform": "satellite",
        "sensor": "synthetic",
...
def create_gridded_geometries(
    variables: Sequence[str] | None = None,
    domain: Domain | None = None,
    time_config: TimeConfig | None = None,
    seed: int | None = 42,
) -> xr.Dataset:
    """Create synthetic gridded datasets (satellite L3 / reanalysis).
    ...
    -------
    xr.Dataset
        Gridded dataset dataset with geometry attribute.
    """
    if variables is None:
        variables = ["NO2"]
    if domain is None:
    ...
    # Import from datasets since structure is similar
    ...
    ds.attrs = {
        "title": "Synthetic Gridded Datasets",
        "geometry": "grid",
        "source": "davinci_monet.tests.synthetic",
        "platform": "satellite",
        "product_level": "L3",
