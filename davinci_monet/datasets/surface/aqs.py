"""EPA Air Quality System (AQS) dataset reader.
...
class AQSReader:
    """Reader for EPA AQS surface dataset data.
    ...
    def open(
        ...
        **kwargs
        ...
        -------
        xr.Dataset
            AQS datasets with dimensions (time, x).
        """
    ...
    def _open_with_monetio(
        ...
        **kwargs: Any,
        ...
        for date in date_range:
            try:
                if daily:
                    df: pd.DataFrame = aqs_module.add_data(date, daily=True, **kwargs)
                else:
                    df = aqs_module.add_data(date, daily=False, **kwargs)
                if not df.empty:
                    ds_list.append(df)
            except Exception:
                continue
    ...
    def _dataframe_to_dataset(self, df: pd.DataFrame) -> xr.Dataset:
        ...
        # AQS data typically has time, site info, and datasets
