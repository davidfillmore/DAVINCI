"""GOES L3 AOD (Aerosol Optical Depth) dataset reader.
...
class GOESL3AODReader:
    """Reader for GOES-ABI L3 AOD satellite datasets.
    ...
    def open(
        ...
        **kwargs: Any,
        ...
        """Open GOES L3 AOD dataset files.
    ...
    def _open_with_monetio(
        ...
        **kwargs: Any,
        ...
        import monetio.sat.goes as goes_module

        files = [str(f) for f in file_paths]

        # monetio.sat.goes expects a single file or list
        if len(files) == 1:
            ds: xr.Dataset = goes_module.open_dataset(files[0], **kwargs)
        else:
            ds_list = []
            for f in files:
                try:
                    ds_i: xr.Dataset = goes_module.open_dataset(f, **kwargs)
                    ds_list.append(ds_i)
