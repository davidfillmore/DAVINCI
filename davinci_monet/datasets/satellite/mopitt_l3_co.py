...
class MOPITTL3COReader:
    """Reader for MOPITT L3 CO satellite datasets.
    ...
    def open(
        ...
        **kwargs: Any,
        ...
        """Open MOPITT L3 CO dataset files.
    ...
    def _open_with_monetio(
        ...
        **kwargs: Any,
        ...
        import monetio.sat._mopitt_l3_mm as mopitt_module

        files = [str(f) for f in file_paths]

        if len(files) == 1:
            ds: xr.Dataset = mopitt_module.open_dataset(files[0], **kwargs)
        else:
            ds_list = []
            for f in files:
                try:
                    ds_i: xr.Dataset = mopitt_module.open_dataset(f, **kwargs)
                    ds_list.append(ds_i)
