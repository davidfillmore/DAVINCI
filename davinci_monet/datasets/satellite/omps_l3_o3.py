"""OMPS L3 Total Ozone dataset reader.
...
class OMPSL3O3Reader:
    """Reader for OMPS L3 total ozone satellite datasets.
    ...
    def open(
        ...
        **kwargs: Any,
        ...
        """Open OMPS L3 ozone dataset files.
    ...
    def _open_with_monetio(
        ...
        **kwargs: Any,
        ...
        import monetio.sat._omps_l3_mm as omps_module

        files = [str(f) for f in file_paths]

        if len(files) == 1:
            ds: xr.Dataset = omps_module.open_dataset(files[0], **kwargs)
        else:
            ds_list = []
            for f in files:
                try:
                    ds_i: xr.Dataset = omps_module.open_dataset(f, **kwargs)
                    ds_list.append(ds_i)
