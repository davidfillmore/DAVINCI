...
_OBS_SOURCE = "tropomi_no2_column"
...
def _make_paired_point(n_time: int = 48, n_site: int = 25) -> xr.Dataset:
    ...
    Variable: NO2 column [mol/m2].  Uses the awkward source key to exercise
    source-name cleaning and SI unit formatting.
    """
...
def _make_multi_source_timeseries(n_time: int = 48, n_site: int = 10) -> list[xr.Dataset]:
    ...
    # Three model sources with slightly different means
    return [
        _src_ds(40.0, "cesm_no2_column"),
        _src_ds(44.0, "wrf"),
        _src_ds(37.0, "airnow"),
    ]
...
def _make_track(n_pts: int = 300) -> xr.Dataset:
    ...
    Variable: O3 [ppbv].  Used by curtain and flight_track.
    """
    ...
    ds = xr.Dataset(
        {
            "O3": (
                ["time"],
                o3,
                {"units": "ppbv", "long_name": "Ozone", "source_label": _AWKWARD_SOURCE},
            )
        },
        coords={
            "time": times,
    ...
    ds.attrs.update({"geometry": "track", "source_label": _AWKWARD_SOURCE})
    return ds
...
def _make_profile(n_lev: int = 80) -> xr.Dataset:
    ...
    ds.attrs.update({"geometry": "profile", "source_label": _AWKWARD_SOURCE})
    return ds
...
def _make_radiation_gridded(n_lon: int = 30, n_lat: int = 15, n_time: int = 3) -> xr.Dataset:
    ...
    Variable: SW radiation [W m-2]; exercises SI unit formatting.
    """
    ...
    lon2d, lat2d = np.meshgrid(lons, lats)
    base = 200.0 + 100.0 * np.cos(np.radians(lat2d))
    ...
    x_vals = _field(10.0)
    y_vals = x_vals + _field(20.0) * 0.15
    ...
    ds = xr.Dataset(
        {
            f"{_OBS_SOURCE}_SW": (
                ...
                {
                    "units": "W m-2",
                    "long_name": "Observed SW",
                    "axis": "x",
                    "source_label": _OBS_SOURCE,
                },
            ),
            f"{_AWKWARD_SOURCE}_SW": (
                ["time", "lat", "lon"],
                y_vals,
                {
                    "units": "W m-2",
...
def make_scatter() -> Path:
    ...
    x_var = f"{_OBS_SOURCE}_NO2"
    y_var = f"{_AWKWARD_SOURCE}_NO2"
    plotter = ScatterPlotter(PlotConfig(title="NO2 Column Scatter"))
...
def make_timeseries_multi_uncertainty() -> Path:
    ...
    plotter = TimeSeriesPlotter(PlotConfig(title="O3 Multi-Source Overlay"))
    series = []
    ...
    plotter2 = TimeSeriesPlotter(PlotConfig(title="O3 Single-Source + Std Band"))
    s0 = series[0]
...
def make_spatial_grid() -> Path:
    """Single-source spatial map: gridded NO2, mol/m2 (pcolormesh path)."""
    ds = _make_gridded()
    plotter = SpatialPlotter(PlotConfig(title="NO2 Column — CESM Grid"))
...
def make_spatial_point() -> Path:
    """Single-source spatial map: point NO2, mol/m2 (scatter path)."""
    from davinci_monet.core.base import PlotSeries
    from davinci_monet.plots.labels import canonical_variable_name

    ds_paired = _make_paired_point(n_time=24, n_site=30)
    # Pull out just the obs variable as a single-source point dataset.
    obs_var = f"{_OBS_SOURCE}_NO2"
    single_ds = xr.Dataset(
        {
            "NO2": (
                ds_paired[obs_var].dims,
                ds_paired[obs_var].values,
                {
                    "units": "mol/m2",
                    "long_name": "TROPOMI NO2 column",
                    "source_label": _OBS_SOURCE,
        ...
        coords={
            "time": ds_paired.coords["time"],
            "site": ds_paired.coords["site"],
            "latitude": ds_paired.coords["latitude"],
            "longitude": ds_paired.coords["longitude"],
        },
    )
    single_ds.attrs.update({"geometry": "point", "source_label": _OBS_SOURCE})

    plotter = SpatialPlotter(PlotConfig(title="NO2 Column — TROPOMI Points"))
    series = [
        PlotSeries(
            dataset=single_ds,
            var_name="NO2",
            canonical=canonical_variable_name(single_ds, "NO2"),
            axis=None,
            source_label=_OBS_SOURCE,
            index=0,
        )
...
def make_spatial_bias() -> Path:
    """Spatial bias map: paired gridded SW radiation, W m-2."""
    ds = _make_radiation_gridded()
    x_var = f"{_OBS_SOURCE}_SW"
    y_var = f"{_AWKWARD_SOURCE}_SW"
    plotter = SpatialBiasPlotter(PlotConfig(title="SW Radiation Bias"))
    series = build_series(ds, x_var, y_var)
    fig = plotter.render(series, lat_var="lat", lon_var="lon", time_average=True)
...
def make_curtain() -> Path:
    """Curtain: 1-D flight track, O3 ppbv, show_var='y' (model values)."""
    ds = _make_curtain_paired()
    x_var = f"{_OBS_SOURCE}_O3"
    y_var = f"{_AWKWARD_SOURCE}_O3"
    plotter = CurtainPlotter(PlotConfig(title="O3 Curtain — Model Field"))
    series = build_series(ds, x_var, y_var)
...
def make_vertical_profile() -> Path:
    """Vertical profile: O3 ppbv vs altitude, binned."""
    ds = _make_profile()
    plotter = VerticalProfilePlotter(PlotConfig(title="O3 Vertical Profile"))
    from davinci_monet.core.base import PlotSeries
    from davinci_monet.plots.labels import canonical_variable_name
...
def make_histogram() -> Path:
    """Histogram: O3 ppbv distribution from track data."""
    ds = _make_track()
    plotter = HistogramPlotter(PlotConfig(title="O3 Distribution"))
    from davinci_monet.core.base import PlotSeries
    from davinci_monet.plots.labels import canonical_variable_name
...
def make_flight_track() -> Path:
    """Flight track: 3-D colored by O3 ppbv."""
    ds = _make_track()
    plotter = FlightTrackPlotter(PlotConfig(title="O3 Flight Track"))
    from davinci_monet.core.base import PlotSeries
    from davinci_monet.plots.labels import canonical_variable_name
