...
Data: Gridded satellite datasets (NO2 column density)
"""
...
def main():
    ...
    ax1.set_title(format_plot_title("Spatial Distribution: TROPOMI L3 NO2 (Geometry)"), fontsize=12)
    plt.tight_layout()

    save_figure(fig1, "15a_satellite_gridded_geometry")
    plt.close(fig1)
    ...
    ax2.set_title(format_plot_title("Spatial Distribution: TROPOMI L3 NO2 (Dataset)"), fontsize=12)
    plt.tight_layout()

    save_figure(fig2, "15b_satellite_gridded_dataset")
    plt.close(fig2)

    # 3. Bias (Dataset - Geometry) with diverging colormap
    ...
    cbar3.set_label(format_plot_title("Bias (Dataset - Geometry) (mol/m²)"), fontsize=10)
