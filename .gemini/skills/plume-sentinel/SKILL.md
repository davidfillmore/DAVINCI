...
7.  **Wait for Completion**: Immediately after launching the terminal, start a background watcher:
    ```bash
    while [ ! -f /tmp/plume-sentinel-done ]; do sleep 5; done && echo "Pipelines complete"
    ```
    Continue informing the user that analyses are running. When the background watcher completes, proceed to the next step.
...
9.  **Monitor & Summarize** (triggered when watcher completes):
    ...
    -   Use `read_file` to inspect each output image.
...
2. Run each config sequentially with `--show-plots` (use absolute paths)
3. Write output file manifest to `/tmp/plume-sentinel-manifest.txt`
4. Touch `/tmp/plume-sentinel-done` to signal completion
5. Return focus to the original window
